...
import frc.robot.commands.TestScoringCommand;
...
public class RobotContainer {
  ...
  public RobotContainer() {
    // score.setDefaultCommand(new InstantCommand(() ->
    // score.goToPosition(Constants.ScoringPos.AMP)));

    //score.setDefaultCommand(new TestScoringCommand(score, testingController));
     ...
     * 
     * HeadingCorTeleopDrive simDrvHeadingCorr = new
     * HeadingCorTeleopDrive(drivebase,
     * () -> MathUtil.applyDeadband(driverController.getLeftY(),
     * OperatorConstants.LEFT_Y_DEADBAND),
     * () -> MathUtil.applyDeadband(driverController.getLeftX(),
     * OperatorConstants.LEFT_X_DEADBAND),
    ...
    TeleopDrive simClosedFieldRel = new TeleopDrive(
        drivebase,
        () -> Math.abs(driverController.getLeftY()) < OperatorConstants.LEFT_Y_DEADBAND ? 0.0
            : driverController.getLeftY(),
        () -> Math.abs(driverController.getLeftX()) < OperatorConstants.LEFT_X_DEADBAND ? 0.0
            : driverController.getLeftX(),
        () -> Math.abs(driverController.getRawAxis(4)) < OperatorConstants.RIGHT_X_DEADBAND ? 0.0
    ...
    // Logger.recordOutput("SwerveStates/ControllerInputLog/RobotContainer/x",
    // y.getAsDouble());
    // Logger.recordOutput("SwerveStates/ControllerInputLog/RobotContainer/y",
    // x.getAsDouble());
    // Logger.recordOutput("SwerveStates/ControllerInputLog/RobotContainer/z",
    // z.getAsDouble());
    // */

    drivebase.setDefaultCommand(RobotBase.isSimulation() ? simClosedFieldRel : closedFieldRel);

    // drivebase.setDefaultCommand(!RobotBase.isSimulation() ? simDrvHeadingCorr :
    // drvHeadingCorr);

    // driverController.rightTrigger().whileFalse();
    // driverController.rightTrigger().whileTrue(closedFieldRelSlow);
    // driverController.start().onTrue(new SequentialCommandGroup(new
    // InstantCommand( () -> drivebase.), new InstantCommand( () ->
    // resetDrive(/*closedFieldRel, simClosedFieldRel*/)).andThen(new
    // WaitCommand(.2)),

    /* reset drive?? */
    driverController.start().onTrue(new SequentialCommandGroup(
        new InstantCommand(() -> drivebase.removeDefaultCommand()),
        new InstantCommand(() -> drivebase.drive(new Translation2d(),
            drivebase.getTargetSpeeds(0, 0, drivebase.getHeading().getSin(),
                drivebase.getHeading().getCos()).omegaRadiansPerSecond,
            true)),
        new WaitCommand(0.2),
        new InstantCommand(
            () -> drivebase.setDefaultCommand(!RobotBase.isSimulation() ? simClosedFieldRel : closedFieldRel))));
    ...
    // new InstantCommand( () ->
    // drivebase.setDefaultCommand(!RobotBase.isSimulation() ? simClosedFieldRel :
    // closedFieldRel))));
  ...
  private void configureBindings() {
    ...
    // intakeSub.intakeNote(5)));

    // driverController.leftBumper().onTrue( new
    // TimedIntakeSetPowerCommand(intakeSub, 10, 1.5));
    ...
    // driverController.leftBumper().onFalse(new InstantCommand( () ->
    // intakeSub.stopIntake() ));

    // driverController.rightBumper().onFalse(new InstantCommand( () ->
    // wristIntake.stopIntake() ));
    ...
    // armWristSub.goToPosition(Height.SOURCE)));
    ...
    /* preclimb */
    // armController.b().onTrue(new InstantCommand(() ->
    // armWristSub.goToPosition(Height.AMP)));
    ...
    // Holding Position
    // armController.x().onTrue(new InstantCommand(() ->
    // armWristSub.goToPosition(Height.HOLD)));
